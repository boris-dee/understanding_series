...
def xyz2AC(atomicNumList,xyz, factor):
    import numpy as np
    ...
    for i in range(num_atoms):
        ...
        Rcov_i = pt.GetRcovalent(a_i.GetAtomicNum())*factor
        for j in range(i+1,num_atoms):
            a_j = mol.GetAtomWithIdx(j)
            Rcov_j = pt.GetRcovalent(a_j.GetAtomicNum())*factor
...
def xyz2mol(atomicNumList,charge,xyz_coordinates,charged_fragments,quick, factor=1.30):

# Get atom connectivity (AC) matrix, list of atomic numbers, molecular charge, 
# and mol object with no connectivity information
    AC,mol = xyz2AC(atomicNumList,xyz_coordinates, factor)
